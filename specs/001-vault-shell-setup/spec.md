...
2. **Given** an existing compatible vault folder, **When** the user opens it, **Then** the system loads it without changing existing CRM entity files.
...
3. **Given** startup with missing app-level settings, **When** the app opens, **Then** the app remains usable by showing vault selection instead of failing startup.
...
- App-level settings are unreadable or corrupted at startup.
...
- **FR-014**: If persistent dependencies are unavailable at startup (vault path unavailable or app settings unreadable), the system MUST degrade to a usable selection flow rather than entering a broken partial state.
